...
async function run() {
  await searchNaverImage("함양문화예술회관 전경");
  await searchNaverImage("함양 상림공원 가을 단풍");
  await searchNaverImage("함양 상림공원 꽃무릇");
  await searchNaverImage("함양 개평한옥마을 일두고택 가을");
  await searchNaverImage("지리산 천왕봉 가을 사계");
}
